...
// import { StyleSheet, View, Text, PanResponder, Dimensions, Image, SafeAreaView, StatusBar, TouchableOpacity } from 'react-native';
// import AsyncStorage from '@react-native-async-storage/async-storage';
...
// const App = () => {
//   const birdYRef = useRef(HEIGHT / 2 - BIRD_HEIGHT / 2);
//   const [pipes, setPipes] = useState([]);
//   const [powerUps, setPowerUps] = useState<PowerUp[]>([]);
//   const [coins, setCoins] = useState([]);
//   const [score, setScore] = useState(0);
...
//       if (gameActive) {
//         const newY = gestureState.moveY - BIRD_HEIGHT / 2;
//         birdYRef.current = Math.max(0, Math.min(newY, HEIGHT - BIRD_HEIGHT));
//       }
//     },
//   }), [gameActive]);
...
//       saveHighScore(score);
//     }
//     birdYRef.current = HEIGHT / 2 - BIRD_HEIGHT / 2;
//     setPipes([]);
//     setPowerUps([]);
//     setCoins([]);
//     setScore(0);
...
//       const newX = powerUp.x - PIPE_SPEED;
//       if (checkCollision(
//         { x: 50, y: birdYRef.current, width: BIRD_WIDTH, height: BIRD_HEIGHT },
//         { x: newX, y: powerUp.y, width: POWER_UP_SIZE, height: POWER_UP_SIZE }
//       )) {
//         activatePowerUp(powerUp.type);
//         return null;
...
//       newCoins = newCoins.map((coin) => {
//         const dx = 50 - coin.x;
//         const dy = birdYRef.current - coin.y;
//         const distance = Math.sqrt(dx * dx + dy * dy);
//         if (distance < 100) {
//           return {
//             ...coin,
...
//     // Check for collisions
//     const birdRect = { x: 50, y: birdYRef.current, width: BIRD_WIDTH, height: BIRD_HEIGHT };
//     const collision = newPipes.some((pipe) => {
//       const upperPipe = { x: pipe.x, y: 0, width: PIPE_WIDTH, height: pipe.gapStart };
//       const lowerPipe = { x: pipe.x, y: pipe.gapStart + PIPE_GAP, width: PIPE_WIDTH, height: HEIGHT - pipe.gapStart - PIPE_GAP };
//       return checkCollision(birdRect, upperPipe) || checkCollision(birdRect, lowerPipe);
...
//     if ((collision && activePowerUp !== 'invincibility') || birdYRef.current <= 0 || birdYRef.current + BIRD_HEIGHT >= HEIGHT) {
//       setGameActive(false);
//     } else {
//       setPipes(newPipes);
//       setPowerUps(newPowerUps);
...
//       setScore(newScore);
//     }
//   }, [pipes, powerUps, coins, score, activePowerUp, generatePipe, generatePowerUp, generateCoin, activatePowerUp]);
...
//         <Image
//           source={require('./assets/bird.png')}
//           style={[styles.bird, { top: birdYRef.current }]}
//         />
//         {pipes.map((pipe) => (
//           <React.Fragment key={pipe.id}>
//             <View
...
//         )}
//         {!gameActive && (
//           <TouchableOpacity style={styles.endScreen} onPress={resetGame}>
//             <View>
//               <Text style={styles.endScreenText}>Game Over</Text>
//               <Text style={styles.endScreenScore}>Score: {score}</Text>
//               <Text style={styles.endScreenCoins}>Coins: {coinsCollected}</Text>
